...
- **Determine the domain** from the task context (e.g., accrual, KPI, project-management, SAP, basket, etc.). If unclear, ask the user. Default to `general` if truly uncategorizable.
- Create your file under `thoughts/shared/handoffs/<domain>/YYYY-MM-DD_HH-MM-SS_description.md`, where:
  - YYYY-MM-DD is today's date
  - HH-MM-SS is the hours, minutes and seconds based on the current time, in 24-hour format (i.e. use `13:00` for `1:00 pm`)
  - `<domain>` is the detected domain (e.g., accrual, kpi, general)
  - description is a brief kebab-case description
- Run `node ${CLAUDE_PLUGIN_ROOT}/scripts/spec_metadata.js` to generate all relevant metadata
- Examples:
    - `thoughts/shared/handoffs/accrual/2025-01-08_13-55-22_parallel-accrual-workflows.md`
...
### 3. Approve
Once all is completed and file is saved, you should respond to the user with the template between <template_response></template_response> XML tags. do NOT include the tags in your response.
...
```bash
/resume_handoff thoughts/shared/handoffs/accrual/2025-01-08_13-44-55_parallel-accrual-workflows.md
```
</example_response>
